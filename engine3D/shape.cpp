...
//@Edited
Shape::Shape(const std::string& fileName, int simplifyFlag, double percent){
	mesh = new Mesh(fileName, simplifyFlag, percent);
	tex = 0;
	isCopy = false;
}


Shape::Shape(const std::string& fileName, int simplifying, double percent, const char* outPath) {
	mesh = new Mesh(fileName, outPath, simplifying, percent);
	tex = 0;
	isCopy = false;
}

//@Edited
Shape::Shape(const std::string& fileName,const std::string& textureFileName, int simplifyFlag, double percent){
	mesh = new Mesh(fileName, simplifyFlag,percent);
