...
MeshSimplification::MeshSimplification(list<OBJIndex> OBJIndices, vector<glm::vec3> vertices, double percent) : m_vertices(vertices), m_OBJIndices(OBJIndices)
{
	// Init edgeVector.
	initEdgeVector();
		...
		for (multimap<int, int>::iterator it2 = it; it2 != result.second; it2++)
		{
			if (it2->second != it->second)
			{
				...
				if (isTriangle(it->second, it2->second))
				{
					...
					glm::vec4 v_tag = glm::vec4(n, -(glm::dot(m_vertices[vertexIndex], n)));
					for (int i = 0; i < 4; i++) 
					{
						for (int j = 0; j < 4; j++) 
						{
...
/*
	...
	glm::vec4 p1 = glm::vec4(m_vertices[e.firstVertex.vertexIndex], 1);
	glm::vec4 p2 = glm::vec4(m_vertices[e.secondVertex.vertexIndex], 1);
	
	double cost;
	if (glm::determinant(addQ1) != 0) {
		glm::mat4 res = glm::inverse(addQ1);
		glm::vec4 vec = a * res;
		e.newPos = glm::vec3(vec);

		glm::vec4 temp = vec * e.edgeQ ;
		e.edgeError = glm::dot(vec, temp);
	}
	else {
		double maxk = 0, mincost = 1000000000;
		glm::vec4 vec;
		glm::vec4 pos;
		for (double k = 0; k <= 1; k = k + 0.5) {
			vec = glm::vec4(1 - k, 1 - k, 1 - k, 1 - k) * p1 + glm::vec4(k, k, k, k) * p2;
			glm::vec4 temp = vec * e.edgeQ;
			cost = glm::dot(vec, temp);
			if (cost < mincost) {
				maxk = k;
				mincost = cost;
				pos = vec;
		...
		e.newPos = glm::vec3(pos);
		e.edgeError = mincost;
	}
*/	// calc new position in the middle
	glm::vec4 vec = glm::vec4((m_vertices[e.firstVertex.vertexIndex] + m_vertices[e.secondVertex.vertexIndex]) / 2.0f, 1.0);
	e.newPos = glm::vec3(vec);

	// temp is a row vector
	glm::vec4 temp = e.edgeQ * vec;
	e.edgeError = glm::dot(vec, temp);

}

//  @tbrief The simplification algorithm.
void MeshSimplification::start(double percent)
{
	int currFaces = m_OBJIndices.size() / 3;
	std::cout << currFaces << std::endl;
	int target = currFaces * percent;

	while (currFaces > target)
	{
		//printEdgeVector(m_edgeVector);
...
vector<glm::vec3> MeshSimplification::getVertices()
{
	return m_vertices;
}
